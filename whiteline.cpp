...
void
WhiteLine::draw(QOpenGLShaderProgram* pOtherProgram) {
    pOtherProgram->bind();
    if(pTexture)
        pTexture->bind();
    ...
    pOtherProgram->enableAttributeArray(vertexLocation);
    pOtherProgram->setAttributeBuffer(vertexLocation, GL_FLOAT, offset, 3, 8*sizeof(float));

    // Offset for normal coordinate
    offset += 3*sizeof(float);
    int normalLocation = pProgram->attributeLocation("vNormal");
    pOtherProgram->enableAttributeArray(normalLocation);
    pOtherProgram->setAttributeBuffer(normalLocation, GL_FLOAT, offset, 3, 8*sizeof(float));

    // Offset for texture coordinate
    offset += 3*sizeof(float);
    int texcoordLocation = pProgram->attributeLocation("vTexture");
    pOtherProgram->enableAttributeArray(texcoordLocation);
    pOtherProgram->setAttributeBuffer(texcoordLocation, GL_FLOAT, offset, 2, 8*sizeof(float));
